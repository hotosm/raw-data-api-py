...
class TestCliIntegration:
    ...
    def run_cli_command(self, args, check=True):
        ...
        if check and result.returncode != 0:
            pytest.fail(f"Command failed with exit code {result.returncode}: {result.stderr}")
    ...
    def test_geojson_download(self):
        ...
        result = self.run_cli_command([
            ...
            "--feature-type", "building",  # Changed from "amenity" to "building" for consistency
            "--out", str(output_file)
        ])
    ...
    def test_extract_option(self):
        ...
        assert output_path is not None, f"Neither {output_file} nor {output_file.with_suffix('.zip')} exist"
    ...
    def test_different_formats_cli(self):
        ...
        assert outputs_exist, f"No output found at {expected_file}, {expected_zip}, or {expected_dir}"
